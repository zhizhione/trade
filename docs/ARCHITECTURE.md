...
Databento `CANCEL`（`action='C'`）表示按该条消息的 `size` 减少指定订单的剩余量，可能是部分撤单；只有剩余量归零时订单才从活动订单簿移除。它不是成交量，也不能直接等同于“全量删除”。实时 ATAS `Delete` 的语义不同：供应商可能只提供订单 ID，系统按活动订单的全部剩余量删除，并保留原始 Delete 事件，不伪造撤单数量。

撤单相关特征和模型只有在历史、实时使用一致的事件语义、可见时间和数据质量时才可用于实盘。必须重点防止以下偏差：

1. 把挂单方向（`B/A`）误当成主动成交方向，或把 `CANCEL` 数量误当成成交量。
2. 把本次撤量误当成订单剩余量，或把 ATAS 全量 Delete 与 Databento 部分 Cancel 混为一谈。
3. 历史按 `ts_event` 使用了实盘尚未收到的事件，造成未来信息泄漏；线上特征只能使用信号时点已经可见的事件（通常以 `ts_recv`/接收时间为边界）。
4. 在实时流丢包、失序、缺少 reset 或只有 L2/MBP 时继续计算未经标记的 L3 撤单和队列特征。

撤单特征应至少区分买卖方向和事件类型，例如 `cancel_bid_qty`、`cancel_ask_qty`、`cancel_imbalance`、`cancel_rate` 及队列前方撤量，并记录原始/推导来源。训练和回测必须过滤 `complete=false`、`crossed=true`、缺口窗口和失同步流。上线前要用同一标准化逻辑对比历史回放与实时适配器的逐事件订单簿状态，并对撤单延迟、丢失、仅 MBP 数据和全量 Delete 做敏感性回测。
