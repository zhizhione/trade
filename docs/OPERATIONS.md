...
将基于 Databento 历史 MBO 的撤单特征或模型接入实盘前，必须完成以下核对：

1. 确认特征读取的是 `CANCEL` 本次撤销量，而不是订单剩余量或成交量；分别统计买方和卖方挂单方向。
2. 确认 ATAS `Delete`（可能没有数量）被标记为全量删除/推导事件，不与 Databento 部分 `C` 混算。
3. 确认线上计算使用接收时点可见事件，回测加入真实采集、标准化和下单延迟；不能只按历史 `ts_event` 对齐。
4. 发现丢序号、缺口、无完整 reset、`DESYNCHRONIZED`、`complete=false` 或 `crossed=true` 时，停止输出 L3 撤单和队列信号。
5. 用同一标准化逻辑做历史回放/实时适配器逐事件订单簿对账，并对撤单丢失、延迟、仅 MBP 和全量 Delete 做敏感性回测。

没有完成上述核对时，历史模型的离线指标不能作为实盘性能预期；相关特征必须带有版本、原始/推导来源和数据质量标记。
